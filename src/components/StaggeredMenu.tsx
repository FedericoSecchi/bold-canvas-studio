...
const StaggeredMenu = ({ isOpen, onClose, navigationItems = [] }: StaggeredMenuProps) => {
  ...
  // Convert navigationItems strings to menu items format
  const items = navigationItems.map((item) => ({
    label: item,
    link: `#${item.toLowerCase()}`,
    ariaLabel: `Navigate to ${item} section`
  }));

  const position = 'right';
  const colors = ['#0a0a0a', '#1a1a1a', '#2a2a2a'];
  const menuButtonColor = '#fff';
  const openMenuButtonColor = '#fff';
  const accentColor = '#82ff00';
  const changeMenuColorOnOpen = false;
  const isFixed = true;
  const closeOnClickAway = true;
  const displaySocials = false;
  const socialItems: never[] = [];
  ...
  }, [isOpen, playOpen, playClose, animateIcon, animateColor, animateText]);
  ...
  // Don't render the toggle button - we use Header's button instead
  // But we need the structure for animations
  return (
    <div
